...
class Camera(nn.Module):
    ...
    def get_dino_feature(self, language_feature_dir):
        features_dir = Path(language_feature_dir) / "dino_features_dim3"
    ...
    def get_language_feature(self, language_feature_dir, feature_level):
        ...
        y, x = torch.meshgrid(
            torch.arange(0, self.image_height), torch.arange(0, self.image_width)
        )
        x = x.reshape(-1, 1)
        y = y.reshape(-1, 1)
        ...
        point_feature = point_feature1.reshape(
